...
def enroll_student(request):
    ...
    turma_id = request.data.get('cod_turma')
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if inscritos >= capacidade:
                return Response({"error": "Turma cheia! Não há vagas disponíveis."},
            ...
            if cursor.fetchone():
                return Response({"error": "Aluno já matriculado nesta turma."},
                                status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"message": "Matrícula realizada com sucesso!"},
